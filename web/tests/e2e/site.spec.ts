  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // the landing-only preview build, so validate it structurally and
        // require it to open in a new tab rather than navigating to it.
        ...
        throw new Error(`#${id}: unexpected href "${href}"`);
      }
    }
  }
});
...
test('internal tab links actually navigate to their view', async ({ page }) => {
  await gotoTab(page, 'overview');
  // The overview hero CTA "Read the docs" links to #docs.
